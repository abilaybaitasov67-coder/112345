...
import { syncGrenadeModels } from './shooter3dGrenades';
...
export class Shooter3dRenderer {
  ...
  private grenadeModels: THREE.Group[] = [];
  ...
  private syncWeapon(world: ShooterWorld) {
    ...
    void loadBlenderWeapon(requestedWeapon).then((model) => {
      ...
      this.weapon = model;
      const hasHands = requestedWeapon === 'ak47' || requestedWeapon === 'm4a4';
  ...
  render(world: ShooterWorld) {
    ...
    syncGrenadeModels(this.scene, this.grenadeModels, world, SCALE);
